...
st.title("🍽️ Sentiment Analysis - Restaurant Reviews (LightGBM)")
st.markdown("### Built by **Ashutosh** | NLP • ML • LightGBM • Streamlit")
...
if os.path.exists(MODEL_PATH) and os.path.exists(VECTORIZER_PATH):
    ...
    st.sidebar.warning("⚠️ No saved model found. Please train one first.")
...
if st.button("Predict Sentiment"):
    if model is None or vectorizer is None:
        st.error("⚠️ Please train or load a model first.")
    ...
    try:
        ...
        if 'Review' not in df_new.columns:
            ...
            if st.button("Predict Batch Sentiments"):
                if model is None or vectorizer is None:
                    st.error("⚠️ Please train or load the model first.")
                else:
